...
Man betrachte die beiden Viererimpulse: 
\begin{equation}
p_{1}=\left(\begin{array}{c}E_{1}\\\overrightarrow{p_{1}}\end{array}\right) \textnormal{ und } p_{2}=\left(\begin{array}{c}E_{2}\\\overrightarrow{p_{2}}\end{array}\right)
\end{equation}
Dabei sind $E_{1}$ und $E_{2}$ die Energien und $\overrightarrow{p_{1}}$ und $\overrightarrow{p_{2}}$ die Impulse der jeweiligen Photonen.

Desweiteren gilt für den Impulsübertrag:
\begin{equation}
q^2=(p_{1}-p_{2})^2
\label{eq:q^2=(p1-p2)^2}
\end{equation}
Da immer $q^2<0$ gilt, definiert man: $Q^2=-q^2$. 

Außerdem gilt: $m^2=Q^2$

Im folgenden wird Gleichung \ref{eq:q^2=(p1-p2)^2} weiter umgestellt.
\begin{equation}
\begin{split}
q^2={}& (p_{1}-p_{2})^2 \\ 
=& \underbrace{p_{1}^2}_{=m_1^2}+\underbrace{p_{2}^2}_{=m_2^2}-2p_{1}p_{2} \\ =& m_1^2+m_2^2-2E_1E_2+2\overrightarrow{p_1}\overrightarrow{p_2} \\ 
=& m_1^2+m_2^2-2E_1E_2+2|\overrightarrow{p_1}||\overrightarrow{p_2}|cos(\vartheta) \\
\end{split}
\end{equation}

Da mit dem Crystal-Ball nur Photonen detektiert werden konnten, wird angenommen, dass es sich bei den beiden Teilchen um Photonen handelt. Daraus folgt das $|\overrightarrow{p_1}|=E_1$ und $|\overrightarrow{p_2}|=E_2$ gilt. Außerdem sind die beiden Massen gleich Null.

\begin{equation}
\begin{split}
\rightarrow 
q^2={}-2E_1E_2(1-cos(\vartheta))
\end{split}
\end{equation}
Damit lässt sich die invariante Masse des Teilchens, welchen in die beiden Photonen zerfallen ist, durch

\begin{equation}
\begin{split}
\Rightarrow{m=\sqrt{2E_1E_2(1-cos(\vartheta))}}
...
Damit wurde schließlich ein zweidimensionales Histogramm angelegt, mit der invarianten Masse auf der x-Achse. Auf der y-Achse wurde die Energie der Photonen aufgetragen, welche in Intervalle mit einer Breite von 25 MeV unterteilt wurden. 
...
\begin{figure}[h!]
	\begin{center}
		...
		Zu erkennen ist der Untergrundfit (Blau), der Gau{\ss}fit (Gr\"un) und die Kombination der beiden (Rot). Alle weiteren Fits mit dieser Bedingung sind in Abbildung \ref{fig:similarenergyallfits} zu sehen.}
