...
Dieses Packet beinhaltete Prozesse, welche durch Pluto, eine Monte-Carlo basierende Simulation, generiert wurden. Insgesamt beinhaltete es 10 Millionen $\pi^0 \rightarrow \gamma \gamma$ Prozesse. Das Verhalten des Crystal-Ball Detektors wurde mittels GEANT4\footnote{Geometry And Tracking}, einer Simulation, welche das Durchdringen von Partikeln durch Materie zu beschreibt, simuliert.

Das Erstellen dieses neuen Paketes nahm sehr viel Zeit in Anspruch, da das Simulieren der Detektoren sehr aufw\"andig war. Daher dauerte das Simulieren von 100000 Prozessen etwa zwei Stunden. Auch von diesen mussten die meisten verworfen werden, weil Photonen entstanden, deren Energie sich nicht \"ahnelte. So kam es, dass selbst das Paket mit 10 Millionen Prozessen nicht genug Photonenpaare mit \"ahnlicher Energie enthielt und da man noch gr\"o{\ss}ere simulierte Pakete vermeiden wollte, wurde nach einem anderen Weg gesucht.
\newline

Dieser war, dass eine neue $\pi^0$-Gun geschrieben wurde.
Bei der die gew\"unschten Bedingungen bereits in Pluto f\"ur einen Filter sorgte und so mussten die Teilchen, die sowieso verworfen w\"aren, gar nicht erst durch Geant simuliert werden. 

Da man aber trotzdem keine Informationen \"uber den Prozess verlieren wollte, musste er Schritt f\"ue Schritt in dieses neue Modell eingebaut werden. Dazu musste man sich den Zerfall nochmal genauer anschauen:
...
Diese Gleichung konnte nach $|p|$ aufgel\"ost werden. Somit war die Masse der beiden Teilchen und ihr Impuls bekannt. Also konnte ihre Energie mit $E=\sqrt{m^2+p^2}$ ausgerechnet werden.
...
In diesem Tool wurde die Energie des Strahls in einem homogenen Interval gew\"urfelt. Dies entspricht nicht der Realit\"at, da die Energieverteilung von Bremsstrahlung nicht gleichm\"a{\ss}ig ist, sondern im gew\"unschtem Interval eher an eine $1/x$-Verteilung erinnerte. Folglich entsprach auch die Energieverteilung der ausgesandten Photonen nicht der Realit\"at. Dies ist allerdings auch nur nebens\"achlich, f\"ur es meine Untersuchungen unwichtig ist, welche Form die Energieverteilung der Photonen ist, solange alle im Experiment m\"oglichen Photonen enstehen k\"onnen.
...
Dazu wurde ein Histogramm mit den Bedingungen aus Kapitel \ref{sec:Varnachlaessigung-der-Detektoren-am-Rand} gef\"ullt, da mit diesen Bedingungen die kleinste Abweichung erzielt wurde. \"Uber dieses Histogramm wurde dann auch wieder mit der Crystal-Ball-Funktion gefittet, um die $\pi^0$ Position zu bestimmen.
...
Da auch dieses mal nur Photonen betrachtet werden sollten, die eine \"ahnliche Energie aufwiesen, mussten sehr viele Zerf\"alle verworfen werden, die diese Bedingung nicht erf\"ullten.
