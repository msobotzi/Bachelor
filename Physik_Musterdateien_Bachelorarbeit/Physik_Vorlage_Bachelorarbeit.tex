...
Dabei war $N$ der Normierungsfaktor, $\bar{x}$ ist der Erwartungswert und $\sigma$ die Standardabweichung der Gau{\ss}funktion. Der Parameter $\alpha$ gab die Position an, an dem die Gau{\ss}verteilung in das Potenzgesetz, mit dem freien Parameter $n$, \"uber geht\cite{NBI15}.
Root stellte diese Funktion gr\"o{\ss}tenteils bereits, lediglich die Normierung musste hinzugef\"ugt werden.

Der Untergrund wurde weiterhin mit einem Polynom vierten Grades ungen\"ahert, bevor die Crystal-Ball-Funktion angewand wurde.
...
Daraufhin wurde das gleiche Verfahren nochmal angewendet, allerdings wurde dieses mal die Bedingung weggelassen, dass die Photonen sich energetisch ähneln sollten. Alle anderen Bedingungen galten weiterhin, so durften die registrierten Teilchen keine Ladung tragen. Die errechnete invariante Masse der photonen wurde in Abbildung \ref{fig:Energy-Intervall-Hist-All-Energys-No-Condition} eingetragen. Die Energieintervalle wurden direkt mit der Crystal-Ball-Funktion gefittet. Zu sehen in Abbildung \ref{fig:allenergyallfits}. 
