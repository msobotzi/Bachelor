...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{Beispiel eines Fits. Es handelt sich dabei um das Energieintervall von 200 MeV bis 225 MeV mit der Bedingung, dass sich die Photonen energetisch \"ahneln. Alle weiteren Fits mit dieser Bedingung sind in Abbildung \ref{fig:similarenergyallfits} zu sehen}
...
Das gef\"ullte Histogramm ist in Abbildung \ref{fig:2D-Hist-Angle-Energy-Position-Pi0} zu sehen. Hier erkennt man direkt, dass Simulierte Daten benutzt wurden, da der $\pi^0$-Peak viel st\"arker ausgepr\"agt war, im Vergleich zu den reellen Daten.
...
Ein weiterer Abspekt der \"Uberpr\"uft wurde, ist die Abh\"angigkeit zwischen der Position des $\pi^0$-Peaks und dem Ort im Target in dem das Pion entsteht und zerf\"allt. Da die Lebenszeit eines Pions nur etwa $8,5*10^{-17}$ Sekunden betr\"agt, w\"urde ein Pion bei Lichtgeschwindigkeit eine Strecke von zur\"ucklegen. Folglich konnte angenommen werden, dass das Pion am gleichem Ort zerf\"allt, an dem es auch entsteht. Hier wurde schlie{\ss}lich ein weiterer Vorteil der Simulation ausgenutzt. Bei einer Simulation sind alle Prozesse genau bekannt. Von jedem Prozess wei{\ss} man demnach auch den genauen Ort. Bei reellen Daten war dies nicht m\"oglich, da der Crystal-Ball keinen Detektor besa{\ss}, um den Ort des Prozesses zu bestimmen. 
